...
class DroneController:
    ...
    def fly_and_precision_land_with_waypoints(self, waypoints, loiter_alt=5, aruco_duration=30):
        ...
        # Final LAND at home
        print("Switching to LAND at home")
        self.vehicle.mode = VehicleMode("LAND")
        while self.vehicle.mode != "LAND":
            self.vehicle.mode = VehicleMode("LAND")
            print("Waiting for LAND...")
...
class DroneNode(Node):
    ...
    def msg_receiver(self, message):
        ...
        try:
            ...
            altitude = DroneController.instance.vehicle.location.global_relative_frame.alt
            if altitude is None:
                altitude = 0.0
            ...
            if ids is not None:
                ...
                for idx, marker_id in enumerate(ids_flat):
                    if int(marker_id) == int(id_to_find):
                        ...
                        # send landing target (only when detected)
                        DroneController.instance.send_land_message(x_ang, y_ang)
            ...
            # --- IMPORTANT: removed all Target Lost handling ---
            # If not found, do nothing (no resend, no mode changes). We only annotate the frame.
            if not found:
...
# Utility to create controller (singleton)
_controller = None
